...
# Open a file and return data for prediction-only EKF runs
def get_file_data_for_prediction(filename):
    data_loader = robot_python_code.DataLoader(filename)
    data_dict = data_loader.load()

    time_list = data_dict["time"]
    control_signal_list = data_dict.get("control_signal", [None] * len(time_list))
    robot_sensor_signal_list = data_dict["robot_sensor_signal"]

    if len(control_signal_list) != len(time_list):
        control_signal_list = [None] * len(time_list)
...
def run_my_model_on_trial(filename, show_plot=True, plot_color="ko"):
    ...
    x_list, y_list, theta_list = motion_model.traj_propagation(
...
def run_my_model_to_predict_distance(filename):
    ...
    motion_model = motion_models.MyMotionModel([0, 0, 0], 0)
    x_list, _, _ = motion_model.traj_propagation(
        time_list, encoder_count_list, steering_angle_list
    )
    distance = x_list[-30]
...
def run_my_model_to_predict_state(filename):
    ...
    motion_model = motion_models.MyMotionModel([0, 0, 0], 0)
    x_list, y_list, theta_list, distance_list = motion_model.traj_propagation(
        time_list, encoder_count_list, steering_angle_list
    )
...
def process_files_and_plot_curve(files_and_data, directory):
    ...
    for row in files_and_data:
        ...
        time_stamp, x_predicted, y_predicted, theta_predicted, distance_predicted = (
            run_my_model_to_predict_state(directory + filename)
        )
...
if __name__ == "__main__":
    ...
    files_and_data = [
        [
            "robot_data_60_0_28_01_26_13_41_44.pkl",
            67 / 100,
    ...
    if False:
        ...
        encoder_counts_to_distance = (
            -2 * math.pi * wheel_radius / encoder_counts_per_revolution
        )

        plt.plot(
            time_list,
            ((np.array(encoder_count_list)) - encoder_count_list[0])
            * encoder_counts_to_distance
            + x_list[0],
            "k",
        )
