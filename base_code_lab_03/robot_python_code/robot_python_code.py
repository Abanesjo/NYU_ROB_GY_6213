    ...
    def get_pose_estimate(self):
        ...
        if ids is not None:
            ...
            for i in range(len(ids)):
                ...
                # Take marker 0 (or choose the one you want)
                rvec = rvecs[0].reshape(3)
                tvec = tvecs[0].reshape(3)

                # Rodrigues -> rotation matrix -> yaw
                R, _ = cv2.Rodrigues(rvec)
                yaw = float(np.arctan2(R[1, 0], R[0, 0]))

                pose_estimate = [float(tvec[0]), float(tvec[1]), float(tvec[2]),
                                float(rvec[0]), float(rvec[1]), yaw]  # NOTE: last is yaw
